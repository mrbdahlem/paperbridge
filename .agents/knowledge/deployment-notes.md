...
- The first ScribbledPage data migration creates assignment, packet, and QR token tables. Runtime assignment APIs require configured database access and intentionally return `503` when `DATABASE_URL` is absent.
- Local Node server commands load `.env` and `.env.local` through `server/env-loader.js`, with shell variables taking precedence. This lets `db:branch:create` write `.env.local` and `db:migrate` consume it without an extra export step.
- Neon branch roles created through the API can connect before they have DDL privileges on `public`. The migration role needs an owner/admin grant such as `CREATE` on the target schema before `npm run db:migrate` can create tables. After migrations run, `scripts/db-migrate.mjs` grants normal table read/write privileges to the runtime role parsed from `DATABASE_URL`.
